...
const partnerLogos = [
  ...
  { id: 4, name: 'Danışman D', hint: 'consultant logo', imgSrc: 'https://picsum.photos/seed/consultantD/100/60' },
      ...
      <div className="container mx-auto max-w-5xl text-center">
        <ScrollAnimationWrapper className="mb-12">
          ...
          <p className="section-subtitle mx-auto max-w-xl">
            Lider teknoloji sağlayıcıları ve çözüm ortaklarıyla işbirliği yaparak müşterilerimize en kapsamlı proje yönetimi çözümlerini sunuyoruz.
          </p>
        ...
        <ScrollAnimationWrapper>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 sm:gap-8 items-center justify-center">
            {partnerLogos.map((logo) => (
              <div 
                key={logo.id} 
                className="p-4 rounded-lg flex items-center justify-center h-20 filter grayscale hover:grayscale-0 hover:bg-card/70 transition-all duration-300 ease-in-out transform hover:scale-105"
                title={logo.name}
              >
                <Image 
                    src={logo.imgSrc} 
